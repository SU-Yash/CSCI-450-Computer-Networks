...
void receiveFrom(int sockfd, sockaddr_storage * their_addr, socklen_t addr_len){
	int numbytes;
...
int main(void)
{
	...
	if ((error = get_socket_fd(&sockfd, "30255")) != 0) {
		return error;
	}

	receiveFrom(sockfd, &their_addr, sizeof their_addr);
